...
//! ```
use bincode;
use fsst::{Compressor, OwnedDecompressor};
...
fn main() {
    ...
    let decompressor_path = format!("{}.fsst.decompressor", args[0].clone());
    let input_path = Path::new(&args[0]);
    let output_path = Path::new(&output_path);
    let decompressed_path = Path::new(&decompressed_path);
    let decompressor_path = Path::new(&decompressor_path);
...

    {
        ...
        let mut compressor_file = File::create(decompressor_path).unwrap();
        compressor_file
            .write_all(
                bincode::serialize(&compressor.decompressor().to_owned())
    ...
    let mut compressor_file = File::open(decompressor_path).unwrap();
    let mut buf = Vec::new();
    compressor_file.read_to_end(&mut buf).expect("Reading compressor content");
    let decompressor: OwnedDecompressor = bincode::deserialize(&buf).expect("Decoding compressor");
    let decompressor = decompressor.to_decompressor();
